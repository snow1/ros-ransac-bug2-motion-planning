...
def init():
    ...
    follow_point = (0, 0)
    seek_point = (0, 0)
    state = BotState.WALL_FOLLOW #GOAL_SEEK
    rospy.loginfo("Starting in Goal Seek Mode111")
    ...
    while not rospy.is_shutdown():
        ...
        if state == BotState.WALL_FOLLOW:
            ...
            if leave_point_dist >= 1 and line_dist <= 0.5:
                seek_point = (x0, y0)
                state = BotState.WALL_FOLLOW #GOAL_SEEK
                rospy.loginfo("Initiating Goal Seek Mod22222")

        elif state == BotState.GOAL_SEEK: #GOAL_SEEK
